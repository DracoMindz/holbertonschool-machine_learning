...
class NeuralNetwork:
    ...
    def gradient_descent(self, X, Y, A1, A2, alpha=0.05):
        ...
        self.__W1 = (self.__W1 - alpha * np.dot(X, (A1 - Y).T).T / X.shape[1])
        self.__W2 = (self.__W2 - alpha * np.dot(X, (A2 - Y).T).T / X.shape[1])
        self.__b1 = self.__b1 - alpha * (A1 - Y).mean()
        self.__b2 = self.__b2 - alpha * (A2 - Y).mean()
